...
const sumSpendTargets = (spendTargets: EdgeSpendTarget[]): string =>
  spendTargets
    .reduce((amount, { nativeAmount }) => {
      if (!nativeAmount) throw new Error(`Invalid spend target amount: ${nativeAmount}`)
...
const fetchFeesFromEdge = async (args: FetchFeesFromEdgeArgs): Promise<Partial<SimpleFeeSettings>> => {
  const response = await args.io.fetch(`${INFO_SERVER_URI}/networkFees/${args.currencyInfo.currencyCode}`)
...
interface FetchFeesFromVendorArgs {
...
const fetchFeesFromVendor = async (args: FetchFeesFromVendorArgs): Promise<Partial<SimpleFeeSettings>> => {
  let fees: Partial<SimpleFeeSettings> = {}

  if (args) {
    const earnComFees = await fetchFeesFromEarnCom(<FetchFeesFromEarnComArgs>args)
    fees = { ...fees, ...earnComFees }
  }

  if (args.mempoolSpaceFeeInfoServer) {
    const mempoolFees = await fetchFeesFromMempoolSpace(<FetchFeesFromMempoolSpaceArgs>args)
...
interface FetchFeesFromEarnComArgs extends FetchFeesFromVendorArgs {
  earnComFeeInfoServer: string
}
 ...
 * @returns {FeeRates}
 */
const fetchFeesFromEarnCom = async (args: FetchFeesFromEarnComArgs): Promise<Partial<FeeRates>> => {
...
interface FetchFeesFromMempoolSpaceArgs extends FetchFeesFromVendorArgs {
  mempoolSpaceFeeInfoServer: string
}
 ...
 */
const fetchFeesFromMempoolSpace = async (args: FetchFeesFromMempoolSpaceArgs): Promise<Partial<FeeRates>> => {
...
export function calcMinerFeePerByte(
  ...
  switch (networkFeeOption) {
    ...
    case 'custom':
      if (!customNetworkFee || customNetworkFee === '0') {
